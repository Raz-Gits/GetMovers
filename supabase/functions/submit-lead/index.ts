...
const ALLOWED_ORIGINS = [
  "https://mcmoversgm.com",
  "https://www.mcmoversgm.com",
  "https://getmoverssite.netlify.app",
];
...
async function sendToHelloMoving(
  ...
  try {
    ...
    const fullUrl = `https://lead.hellomoving.com/LEADSGWHTTP.lidgw?API_ID=${encodeURIComponent(apiId)}&MOVERREF=${encodeURIComponent(moverRef)}`;
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ success: true, dbSaved: true }),
      {
        headers: {
          ...corsHeaders,
